...
def fetch_playlists(access_token: str) -> list[dict[str, Any]]:
    """
    Fetches up to 50 of the user's playlists in a single API call
    and filters the data to include just the essentials.
    """
    ...
    try:
        ...
        for playlist in raw_json.get("items", []):
            # FIX: Spotify's playlist object uses "tracks", not "items", for these values
            ...
            filtered_playlists.append({
                ...
                "trackCount": tracks_info.get("total"),
                "tracksApiUrl": tracks_info.get("href")
            })

        return filtered_playlists

    except requests.exceptions.RequestException as error:
        print(f"Failed to fetch playlists: {error}")
        return []


def fetch_playlist_tracks(playlist_id: str, access_token: str) -> list[dict[str, Any]]:
    """
    Fetches all items (songs) from a specific playlist using pagination.
    """
    all_tracks = []
    # Spotify's endpoint for getting items inside a specific playlist
    next_url = f"https://api.spotify.com/v1/playlists/{playlist_id}/tracks"
    headers = {"Authorization": f"Bearer {access_token}"}

    try:
        # Loop to handle pagination (fetch 100 items at a time until done)
        while next_url:
            ...
            for item in data.get("items", []):
                ...
                # Skip if the track object is empty (sometimes happens with local files)
                if not track:
                    continue  
                
                # Format multiple artists into a single string
                artists = [artist.get("name") for artist in track.get("artists", [])]
                artist_string = ", ".join(artists) if artists else "Unknown"
            ...
            # Move to the next page of results, or None if we are at the end
